...
Vector2 Vector2::Lerp(Vector2 b, float amountToB)
{
	return *this * (1 - amountToB) + b * amountToB;
}
...
const GLfloat& Vector3::operator [] (unsigned int idx) const
{
	return (&x)[idx];
}
...
Vector3 Vector3::Lerp(Vector3 b, float amountToB)
{
	return *this * (1 - amountToB) + b * amountToB;
}
...
Vector4 Vector4::Lerp(Vector4 b, float amountToB)
{
	return *this * (1 - amountToB) + b * amountToB;
}
...
Matrix& Matrix::SetInvPerspective(GLfloat fovY, GLfloat aspect, GLfloat nearPlane, GLfloat farPlane)
{
	GLfloat height = 2.0f * nearPlane * tanf(fovY * 0.5f);
	GLfloat width = height * aspect;
	GLfloat n2 = 2.0f * nearPlane;
	...
	GLfloat xinv = width / n2;
	GLfloat yinv = height / n2;
	GLfloat z = (farPlane + nearPlane) * rcpnmf;
	GLfloat winv = 1.f / (farPlane * rcpnmf * n2);
